 ...
 * 也就是说，SnowFlake中的dataCenterId && machineId为第一次创建IdWorker时传入的参数，之后传入的参数将无效。
 *
 * <p>
 * 2. 每次创建一个SnowFlake，可以使用不同的dataCenterId 和 machineId来区分（需保证不会有第二个使用相同的dataCenterId && machineId）
 * <p>
 * 使用SnowFlake，每次自行创建一个SnowFlake对象进行生成ID，需保证全局无相同的dataCenterId && machineId 的SnowFlake。
 * （可以构建一个数据库表，存放所有的dataCenterId && machineId，以及创建它的相关类，既可以保证全局唯一，也可以在宕机重启时重新构建）
 *
 * @author guodongzhang
 */
